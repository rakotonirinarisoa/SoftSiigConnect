...
function modalREJET(id) {
    ...
    $.ajax({
        type: "POST",
        url: Origin + '/TraitementJustif/GetIsMotif',
        data: formData,
        cache: false,
        contentType: false,
        processData: false,
...
$(`[data-action="ANNULMANDAT"]`).click(function () {
    let user = $("#Motif").val();
    if (!user) {
        alert("Veuillez renseigner le motif du rejet avant l'annulation du justificatif. ");
        return;
    }
    ...
    $.ajax({
        type: "POST",
        url: Origin + '/TraitementJustif/AnnulationJustif',
        data: formData,
        cache: false,
        contentType: false,
        processData: false,
