...
function modalREJET(id) {
    ...
    $.ajax({
        type: "POST",
        url: Origin + '/TraitementComplement/GetIsMotif',
        data: formData,
        cache: false,
        contentType: false,
        processData: false,
...
$(`[data-action="ANNULMANDAT"]`).click(function () {
    let user = $("#Motif").val();
    if (!user) {
        alert("Veuillez renseigner le motif du rejet avant l'annulation du complément. ");
        return;
    }
    ...
    $.ajax({
        type: "POST",
        url: Origin + '/TraitementComplement/AnnulationJustif',
        data: formData,
        cache: false,
        contentType: false,
        processData: false,
